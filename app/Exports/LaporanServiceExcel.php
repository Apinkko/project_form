...
class LaporanServiceExcel implements FromCollection,WithHeadings
{
    ...
    public function collection()
    {
      ...
      if($dept == 1){
        $service = $service->whereIn('service.user_id', $user_id)->orwhere('c.jenis_inventaris_id', 4);
      }else if($dept == 4){
        $service = $service->whereIn('service.user_id', $user_id)->orwhere('c.jenis_inventaris_id', 3);
      }else if($dept == 7) {
        // all service
        // $service = $service;

        // with price
        $service = $service->where('service.biaya_service','<>', '0');
      }else {
        $service = $service->whereIn('service.user_id', $user_id);
      }

      $service = $service->select('a.nama as pemohon','service.no_tiket','b.department', 'd.jenis_inventaris', 'c.nama as inventaris', 'service.service',
